...
out:
	for atomic.LoadInt32(&p.disconnect) == 0 {
		...
		case *lnwire.ZkPayProof:
			...
			// zkchLog.Info("\npayment length before generation: ", len(payment))
			// zkchLog.Info("\nmerchstate length before generation:", len(merchStateBytes))
			// zkchLog.Info("\nchannelStateBytes length before generation:", len(channelStateBytes))
			// zkchLog.Info("\n\n close token length before generation: ", len(closeTokenBytes))

			// closeToken, merchState, err = libbolt.BidirectionalPayVerifyPaymentProof(channelState, payment, merchState)

			// closeTokenBytes, err = json.Marshal(closeToken)

			// zkchLog.Info("\n\n close token after generation has length: ", len(closeTokenBytes))

			// // darius TODO: I'm not sure if merchState needs to get updated.
			// // open the zkchanneldb to load merchState
			// zkMerchDB, err = zkchanneldb.SetupZkMerchDB()
			// // save updated merchState
			// saveMerchStateBytes, _ := json.Marshal(merchState)
		...
		case *lnwire.ZkPayClose:
			...
			// // To closeToken from rpc message
			// var closeToken libbolt.Signature
			// err = json.Unmarshal(msg.CloseToken, &closeToken)

			// // zkDB load custState, channelState, and newCustState
			...
			// // read custNewState from ZkCustDB
			// var newCustStateBytes []byte
			// err = zkCustDB.View(func(tx *bolt.Tx) error {
			// 	c := tx.Bucket(zkchanneldb.CustBucket).Cursor()
			// 	_, v := c.Seek([]byte("newCustStateKey"))
			// 	newCustStateBytes = v
			// 	return nil
			// })
			// if err != nil {
			...
			// var newCustState libbolt.CustState
			// err = json.Unmarshal(newCustStateBytes, &newCustState)

			// zkCustDB.Close()

			// zkchLog.Info("\nlength of 'newCustStateBytes' AFTER receive CLOSETOKEN: ", len(newCustStateBytes))

			// revokeToken, custState, err := libbolt.BidirectionalPayGenerateRevokeToken(channelState, custState, newCustState, closeToken)

			// _ = revokeToken
			// // zkDB update CustState
			// zkCustDB, err = zkchanneldb.SetupZkCustDB()
			// saveCustStateBytes, _ := json.Marshal(custState)
			// zkchanneldb.AddCustState(zkCustDB, saveCustStateBytes)
			// zkCustDB.Close()

			// // send revokeToken to merch
			// revokeTokenBytes, err := json.Marshal(revokeToken)
		...
		case *lnwire.ZkPayRevoke:
			...
			// // Read revokeToken from rpc message
			// var revokeToken libbolt.RevokeToken
			// err = json.Unmarshal(msg.RevokeToken, &revokeToken)
		...
		case *lnwire.ZkPayToken:
			...
			// if isTokenValid {
			// 	fmt.Println("received payToken is valid!")
			// } else {
			// 	fmt.Println("received payToken is invalid!")
			// }
