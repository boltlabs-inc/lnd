...
// OpenZkChannel attempts opens a zkchannel with a merchant.
// ########### ln-mpc ###########
func (r *rpcServer) OpenZkChannel(ctx context.Context,
	in *lnrpc.OpenZkChannelRequest) (*lnrpc.OpenZkChannelResponse, error) {

	zkchLog.Debugf("OpenZkChannel initiated with peer(%s)", in.PubKey)
	...
	// With all initial validation complete, we'll now request that the
	// server disconnects from the peer.
	if err := r.server.OpenZkChannel(peerPubKey, in.MerchPubKey, in.CustBalance, in.MerchBalance); err != nil {
...
func (r *rpcServer) CloseZkChannel(ctx context.Context,
	...
	zkchLog.Debugf("CloseZkChannel initiated with peer(%s)", in.PubKey)
