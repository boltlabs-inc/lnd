...
func (z *zkChannelManager) initZkEstablish(inputSats int64, custUtxoTxIdLe string, index uint32, custInputSk string, custStateSk string, custPayoutSk string, changePubKey string, merchPubKey string, zkChannelName string, custBal int64, merchBal int64, feeCC int64, feeMC int64, minFee int64, maxFee int64, p lnpeer.Peer) error {
	...
	err = zkchanneldb.AddCustField(zkCustDB, zkChannelName, channelToken, "channelTokenKey")
	if err != nil {
		zkchLog.Error(err)
		return err
	}

	err = zkchanneldb.AddCustField(zkCustDB, zkChannelName, merchPk, "merchPkKey")
	if err != nil {
		zkchLog.Error(err)
		return err
	}
	...
	err = zkchanneldb.AddCustField(zkCustDB, zkChannelName, feeCC, "feeCCKey")
	if err != nil {
		zkchLog.Error(err)
		return err
	}

	err = zkchanneldb.AddCustField(zkCustDB, zkChannelName, feeMC, "feeMCKey")
	if err != nil {
		zkchLog.Error(err)
		return err
	}

	err = zkchanneldb.AddCustField(zkCustDB, zkChannelName, minFee, "minFeeKey")
	if err != nil {
		zkchLog.Error(err)
		return err
	}

	err = zkchanneldb.AddCustField(zkCustDB, zkChannelName, maxFee, "maxFeeKey")
	if err != nil {
		zkchLog.Error(err)
		return err
	}

	err = zkchanneldb.AddCustField(zkCustDB, zkChannelName, escrowTxid, "escrowTxidKey")
	if err != nil {
		zkchLog.Error(err)
		return err
	}

	err = zkchanneldb.AddCustField(zkCustDB, zkChannelName, escrowPrevout, "escrowPrevoutKey")
	if err != nil {
		zkchLog.Error(err)
		return err
	}

	err = zkchanneldb.AddCustField(zkCustDB, zkChannelName, signedEscrowTx, "signedEscrowTxKey")
	if err != nil {
		zkchLog.Error(err)
		return err
	}
...
func (z *zkChannelManager) processZkEstablishAccept(msg *lnwire.ZkEstablishAccept, p lnpeer.Peer, zkChannelName string) {
	...
	err = zkchanneldb.AddCustField(zkCustDB, zkChannelName, channelState, "channelStateKey")
	if err != nil {
		z.failEstablishFlow(p, err)
		return
	}
...
func (z *zkChannelManager) advanceCustomerStateAfterConfirmations(notifier chainntnfs.ChainNotifier, escrowTxid string, pkScript []byte, zkChannelName string, p lnpeer.Peer) {
	...
	err = zkchanneldb.AddCustField(zkCustDB, zkChannelName, false, "closeInitiatedKey")
	if err != nil {
		zkchLog.Error(err)
		return
	}
...
func (z *zkChannelManager) processZkEstablishFundingConfirmed(msg *lnwire.ZkEstablishFundingConfirmed, p lnpeer.Peer, zkChannelName string) {
	...
	err = zkchanneldb.AddCustField(zkCustDB, zkChannelName, state, "stateKey")
	if err != nil {
		z.failEstablishFlow(p, err)
		return
	}
...
func (z *zkChannelManager) InitZkPay(p lnpeer.Peer, zkChannelName string, amount int64) error {
	...
	err = zkchanneldb.AddCustField(zkCustDB, zkChannelName, revState, "revStateKey")
	if err != nil {
		zkchLog.Error(err)
		return err
	}

	err = zkchanneldb.AddCustField(zkCustDB, zkChannelName, newState, "newStateKey")
	if err != nil {
		zkchLog.Error(err)
		return err
	}

	err = zkchanneldb.AddCustField(zkCustDB, zkChannelName, oldState, "oldStateKey")
	if err != nil {
		zkchLog.Error(err)
		return err
	}

	err = zkchanneldb.AddCustField(zkCustDB, zkChannelName, amount, "amountKey")
	if err != nil {
		zkchLog.Error(err)
		return err
	}
...
func GetSignedCustCloseTxs(zkChannelName string, closeEscrow bool) (CloseEscrowTx string, CloseEscrowTxid string, err error) {
	...
	err = zkchanneldb.AddCustField(zkCustDB, zkChannelName, true, "closeInitiatedKey")
	if err != nil {
		zkchLog.Error(err)
		return "", "", err
	}
