...
Adapted from [LND Tutorial](https://dev.lightning.community/tutorial/01-lncli)

### Introduction

This tutorial is designed to walk you through how to set up a zero-knowledge channel (zkchannel), using `zklnd`. `zklnd` is an extension of `lnd` that uses secure multi-party computation to allow customers to make unlinkable payments to a merchant. For more information, read the [TODO: Add link 'zkchannels blog post'](emptyLink).
